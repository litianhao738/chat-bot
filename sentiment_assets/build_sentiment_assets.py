#!/usr/bin/env python3
"""
Build lightweight sentiment assets from the uploaded Kaggle social-media dataset.
...
DEFAULT_DATASET_CANDIDATES = [
    str(ROOT_DIR / "sentiment_assets" / "input" / "sentimentdataset.csv"),
]
DEFAULT_OUTPUT_DIR = str(ROOT_DIR / "sentiment_assets" / "output")
DEFAULT_CLEANED_PATH = str(ROOT_DIR / "sentiment_assets" / "output" / "sentiment_cleaned.jsonl")
...
def resolve_output_path(output_dir: str, configured_path: str, default_path: str) -> str:
    configured = safe_text(configured_path)
    if configured and configured != default_path:
        return configured
    return str(Path(output_dir) / Path(default_path).name)
...
def main() -> None:
    ...
    cleaned_path = resolve_output_path(args.output_dir, args.cleaned_path, DEFAULT_CLEANED_PATH)
    summary_path = resolve_output_path(args.output_dir, args.summary_path, DEFAULT_SUMMARY_PATH)
    keywords_path = resolve_output_path(args.output_dir, args.keywords_path, DEFAULT_KEYWORDS_PATH)
    ...
    write_json(keywords_path, keyword_profiles)
